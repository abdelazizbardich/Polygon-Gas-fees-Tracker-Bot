...
async function getData(message: { reply: (arg0: { content: string }) => void }):Promise<void>{ // send api request to polygon gas fees api
    try {
...
async function waitUntil(time:Number,message: { reply: (arg0: { content: string }) => void }):Promise<void> { // Set a time interval waiter
